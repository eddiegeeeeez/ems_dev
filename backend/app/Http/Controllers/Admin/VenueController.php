...
class VenueController extends Controller
{
    ...
    public function store(Request $request): JsonResponse
    {
        try {
            ...
            return response()->json([
                ...
                'venue' => $venue->load('college')
            ], 201);
        } catch (ValidationException $e) {
            return response()->json(['errors' => $e->errors()], 422);
        } catch (\Exception $e) {
            Log::error('Failed to create venue: ' . $e->getMessage(), ['exception' => $e]);
            return response()->json(['error' => 'Failed to create venue', 'details' => $e->getMessage()], 500);
        }
    }
    ...
    {
        try {
            $venue->load('college', 'equipment', 'bookings');
            $stats = [
